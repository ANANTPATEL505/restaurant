          ...
          <div className="flex gap-2">
            {["week", "month", "year"].map(p => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition capitalize ${period === p ? "bg-[#800020] text-white" : "border border-gray-200 text-gray-600 hover:border-[#800020]"
                  }`}
              >
                {p}
              </button>
          ...
          <div className="p-8 space-y-8">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={period === "month" ? mockMonthly : mockWeekly} barSize={32}>
                  ...
                  <ResponsiveContainer width={180} height={180}>
                    <PieChart>
                      <Pie data={charts.categoryBreakdown?.length ? charts.categoryBreakdown : mockCategories} cx="50%" cy="50%" outerRadius={80} dataKey="value" label={({ value }) => `${value}`} labelLine={false}>
                        {(charts.categoryBreakdown || mockCategories).map((_: any, i: number) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                      </Pie>
                    </PieChart>
                  </ResponsiveContainer>
